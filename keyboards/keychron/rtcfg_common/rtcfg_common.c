...
// Board-independent core of the runtime-config (rtcfg) feature. Driven by the
// companion host app (q1config.py CLI + q1config.html WebHID GUI); wire protocol
// documented in that repo's PROTOCOL.md. Custom raw HID command byte 0xAC is
// routed here via the keychron_common.c via_command_user() hook (which this file
// overrides). Each board keymap supplies only keymaps[]/encoder_map[] and calls
// rtcfg_process_record() from its process_record_user().
#include "raw_hid.h"
#include "action_util.h"
#include "process_combo.h"
#include "process_key_override.h"
#include "rtcfg_common.h"
// State indicators paint the per-key backlight; the API differs between RGB and
// single-color (white) boards. White/LED-matrix variants build with no color.
...
// Layer whose activation lights the FN-layer state indicator. Defaults to the
// WIN_FN slot of the standard 4-layer Keychron keymaps; boards with a different
// layer order can override this in their keymap config.h.
...
#define COMBO_SLOT_COUNT RTCFG_COMBO_COUNT
...
// Custom command byte 0xAC, routed via keychron_common.c's via_command_user() hook.
...
// primary key doesn't wait out the tapping term. Called from the board keymap's
// process_record_user(); returns false to consume the event.
...
// accessors (declared in rtcfg_common.h) to avoid coupling to user_config_t's layout.
...
// the keymap, so the keymap can't override the weak combo_count()/combo_get()
// (same translation unit). Instead we expose the full-size key_combos[] that the
// stock combo_count_raw()/combo_get_raw() serve, and rebuild it from user_config
// at runtime. Disabled/invalid slots get an empty key list (keys[0] == COMBO_END)
// so they never match. RAM key pointers are fine: this is an ARM MCU, where
// pgm_read_word is a plain memory read.
//
// key_combos[] itself is defined by the board keymap (the keymap TU owns the
// table QMK's combo introspection takes sizeof of); we fill it here.
extern combo_t key_combos[COMBO_SLOT_COUNT];
